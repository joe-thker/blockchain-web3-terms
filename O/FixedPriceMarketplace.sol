 ...
 * @notice Sellers list ERC‑721 tokens at a fixed price; buyers pay exactly that ETH.
 *         Marketplace takes a configurable fee (in basis points).
 *
 * FIX: Replaced the Unicode “≤” in the revert string with ASCII "<=",
 *      so the `require` now uses "Fee<=10%" and compiles under Solidity ^0.8.20.
 */
contract FixedPriceMarketplace is ReentrancyGuard, Ownable {
    ...
    function buyItem(address token, uint256 tokenId) external payable nonReentrant {
        ...
        // pay seller
        (bool sentSeller, ) = payable(lst.seller).call{value: vend}("");
