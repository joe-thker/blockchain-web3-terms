...
/**
 * @title FiatOnRampToken
 * @dev ERC20 token that acts as a fiat on-ramp. Users deposit ETH to receive tokens
 * at a fixed conversion rate (tokensPerEth). The owner can update the rate and withdraw ETH.
 */
contract FiatOnRampToken is ERC20, Ownable {
    ...
    /**
     * @dev Allows users to deposit ETH and receive tokens based on the conversion rate.
     */
    function deposit() public payable {
        require(msg.value > 0, "Must send ETH");
        // Calculate token amount to mint: (msg.value in wei * tokensPerEth) / 1 ether
